"""
Myntra Ecommerce Adapter
========================
Modular adapter for Myntra. Handles URL detection, product ID extraction,
...
class MyntraAdapter(BaseEcommerceAdapter):
    store_name = "myntra"
    display_name = "Myntra"
    logo_icon = "myntra"

    def __init__(self):
        self._scraper = MyntraScraper()
    ...
    async def search_matching_product(
        ...
        """
        Search Myntra for matching product with strict variant verification.
        """
        try:
            ...
            search_url = f"https://www.myntra.com/{encoded}"
            ...
            # Myntra product card elements
            items = soup.select("li.product-base")
            for item in items[:6]:
                brand_elem = item.select_one("h3.product-brand")
                title_elem = item.select_one("h4.product-product")
                cand_brand = brand_elem.get_text(strip=True) if brand_elem else ""
                cand_sub = title_elem.get_text(strip=True) if title_elem else ""
                cand_title = f"{cand_brand} {cand_sub}".strip()
                ...
                # Price extraction
                price_elem = item.select_one("span.product-discountedPrice, div.product-price span")
                clean_p = re.sub(r"[^\d]", "", price_elem.get_text()) if price_elem else ""
                cand_price = float(clean_p) if clean_p else None

                # Strike MRP
                mrp_elem = item.select_one("span.product-strike")
                cand_mrp = None
                if mrp_elem:
                ...
                cand_url = f"https://www.myntra.com/{href}" if href and not href.startswith("http") else (href or search_url)
                ...
                return {
                    "store": self.store_name,
                    "seller_name": "Myntra Authenticated Partner",
                    "seller_id": pid or "myntra_seller",
                    "price": cand_price,
                    "original_price": cand_mrp,
                    "shipping_price": 0.0,
                    "delivery_text": "Free Delivery (Insider & Orders > ₹799)",
                    "coupon_text": "Coupon Discounts on Checkout",
                    "url": cand_url,
            ...
            return {
                ...
                "match_reason": f"No exact match found on Myntra ({best_reason})",
                "variants": [],
            }
        except Exception as e:
            logger.debug(f"Myntra matching search error: {e}")
        return None
