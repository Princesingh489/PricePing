"""
Flipkart Ecommerce Adapter
==========================
Modular adapter for Flipkart India. Handles URL detection, PID/FSN extraction,
...
class FlipkartAdapter(BaseEcommerceAdapter):
    store_name = "flipkart"
    display_name = "Flipkart"
    logo_icon = "flipkart"

    def __init__(self):
        self._scraper = FlipkartScraper()

    def detect_url(self, url: str) -> bool:
        if not url:
            return False
        u = url.lower()
        return any(domain in u for domain in ["flipkart.com", "dl.flipkart.com", "fkrt.it"])
    ...
    async def fetch_product(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "seller": getattr(result, "seller", "Flipkart Assured"),
            "url": url,
        }
    ...
    async def search_matching_product(
        ...
        """
        Search Flipkart for matching product with strict variant verification.
        """
        try:
            ...
            cards = soup.select("div._1AtVbE, div[data-id], div._75nlfW, a._1fQZEK")
            for card in cards[:8]:
                # Title
                title_elem = card.select_one("div._4rR01T, a.s1Q9rs, div.KzDlHZ, a.wjcEIp")
                if not title_elem:
                    continue
                ...
                # Product URL / ID
                link_elem = card.select_one("a._1fQZEK, a.s1Q9rs, a.CGtC58, a[href*='/p/']")
                href = link_elem.get("href") if link_elem else None
                cand_url = f"https://www.flipkart.com{href}" if href and href.startswith("/") else (href or search_url)
                pid = self.extract_product_id(cand_url) or card.get("data-id")
                ...
                # F-Assured / Delivery
                is_assured = bool(card.select_one("img[src*='fassured'], div._32eT7m"))
                delivery_text = "Free Flipkart Assured Delivery" if is_assured else "Standard Delivery"
                ...
                return {
                    "store": self.store_name,
                    "seller_name": "Flipkart Assured Seller",
                    "seller_id": pid or "flipkart_seller",
                    "price": cand_price,
                    "original_price": cand_mrp,
                    "shipping_price": 0.0,
                    "delivery_text": delivery_text,
                    "coupon_text": "Extra Bank Offers & SuperCoins",
                    "url": cand_url,
            ...
            return {
                ...
                "match_reason": f"No exact match found on Flipkart ({best_reason})",
                "variants": [],
            }
        except Exception as e:
            logger.debug(f"Flipkart matching search error: {e}")
        return None
