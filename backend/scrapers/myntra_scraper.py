"""
Myntra India Scraper
====================
High-accuracy scraper for Myntra.com using multi-source verification (DOM, JSON-LD, Metadata), candidate price classification, and high-res image extraction.
"""
import re
import json
import logging
...
class MyntraScraper(BaseScraper):
    ...
    def extract_product_id(self, url: str) -> Optional[str]:
        """Extract Myntra style/product ID."""
        if not url:
            return None
        patterns = [
            r'/(\d+)/buy',
    ...
    def get_main_container(self, soup: BeautifulSoup) -> Optional[Tag]:
        return DomExtractor.get_main_container(soup, PlatformEnum.myntra)
    ...
    def extract_from_html(self, html: str, url: str) -> ExtractionResult:
        ...
        # 1. Title Extraction
        title = None
        brand_elem = target.select_one("h1.pdp-title")
        name_elem = target.select_one("h1.pdp-name, h2.pdp-name")
        ...
        disc_elem = target.select_one("span.pdp-discount")
        if disc_elem:
        ...
        # 5. Availability
        availability = DomExtractor.extract_availability(target, PlatformEnum.myntra)
    ...
    async def extract_product(self, url: str) -> ExtractionResult:
        ...
        try:
            ...
            html, method = await PlaywrightManager.fetch_html(
                url,
                wait_selector="h1.pdp-title, span.pdp-price, div.pdp-details",
                timeout_ms=15000,
            )
        except Exception as e:
            logger.debug(f"Playwright fetch failed for Myntra: {e}")
        ...
        if not html:
            ...
            try:
                ...
                logger.error(f"HTTP fallback failed for Myntra: {e}")
