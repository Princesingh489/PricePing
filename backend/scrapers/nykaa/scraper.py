...
from scrapers.nykaa.extractors import NykaaExtractor
from scrapers.nykaa.validators import NykaaValidator
...
class NykaaScraper(BaseScraper):
    ...
    def extract_product_id(self, url: str) -> Optional[str]:
        """Extract Nykaa product identifier (e.g. 231234 or productId=...)."""
        if not url:
            return None
        m = re.search(r'/p/(\d+)', url)
        if m:
            return m.group(1)
        m = re.search(r'productId=(\d+)', url, re.IGNORECASE)
        if m:
            return m.group(1)
        return None
    ...
    def extract_from_html(self, html: str, url: str) -> ExtractionResult:
        ...
        # 2. Candidates
        candidates, meta_data = NykaaExtractor.extract_candidates(soup, target)
        if not title and meta_data.get("json_ld_name"):
            title = SharedNormalizer.clean_title(meta_data["json_ld_name"])
        brand = meta_data.get("json_ld_brand")

        # 3. Price Ranking & Discount
        raw_price, raw_mrp = NykaaValidator.filter_and_rank_prices(candidates)
        ...
        image_url = NykaaExtractor.extract_image(target, soup) or meta_data.get("json_ld_image")
        images = NykaaExtractor.extract_images(soup)
        ...
        variant = NykaaExtractor.extract_variant(soup, target, url)
        variants = NykaaExtractor.extract_variants(
            soup=soup,
            target=target,
            default_price=current_price,
            default_mrp=original_price,
        )
        avail_str, is_buybox_active = NykaaExtractor.extract_availability(soup, target)
